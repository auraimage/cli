...
Per-project Secret Keys, project-scoped `aura init`, and richer `aura upload`.
...
- `aura upload <path>` accepts a single image file *or* a directory. Directory mode recurses into subdirectories, skipping dot-dirs (`.git`, `.next`, …) and `node_modules` / `dist` / `build`. Uploaded filenames preserve the path relative to the upload root.
